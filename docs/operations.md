...
`fathom-algo-search-lab` is centered on algorithms work in OCaml. The technical target is: Package an OCaml local lab for search analysis with append-only fixtures, checkpoint recovery checks, and documented operating limits.
...
The score is deterministic. The current threshold is 181. Risk has a penalty of 6, latency has a penalty of 2, and weight contributes a bonus of 4.
